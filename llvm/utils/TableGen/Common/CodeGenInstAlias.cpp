...
static Expected<ResultOperand> matchSimpleOperand(Init *Arg,
                                                  StringInit *ArgName,
                                                  Record *Op,
                                                  CodeGenTarget &T) {
  if (Op->isSubClassOf("RegisterClass") ||
    ...
    if (auto *ArgDef = dyn_cast<DefInit>(Arg);
        ArgDef && ArgDef->getDef()->isSubClassOf("Register")) {
      if (!T.getRegisterClass(OpRC).contains(
              T.getRegBank().getReg(ArgDef->getDef())))
        return createStringError(
            "register argument " + ArgDef->getDef()->getName() +
            " is not a member of operand register class " + OpRC->getName());
      if (ArgName)
        return createStringError("register argument must not have a name");
      return ResultOperand::createRegister(ArgDef->getDef());
    }

    if (auto *ArgDef = dyn_cast<DefInit>(Arg);
        ArgDef && ArgDef->getDef()->getName() == "zero_reg") {
      if (ArgName)
        return createStringError("register argument must not have a name");
  ...
  if (Op->isSubClassOf("Operand")) {
    if (IntInit *ArgInt = dyn_cast_or_null<IntInit>(
            Arg->convertInitializerTo(IntRecTy::get(Arg->getRecordKeeper())))) {
      if (ArgName)
        return createStringError("immediate argument must not have a name");
...
static Expected<ResultOperand> matchComplexOperand(Init *Arg,
                                                   StringInit *ArgName,
                                                   Record *Op,
                                                   CodeGenTarget &T) {
  assert(Op->isSubClassOf("Operand"));
  auto *ArgDef = dyn_cast<DefInit>(Arg);
...
CodeGenInstAlias::CodeGenInstAlias(Record *R, CodeGenTarget &T) : TheDef(R) {
  ...
  for (auto [OpIdx, OpInfo] : enumerate(ResultInst->Operands)) {
    ...
    if (Op->isSubClassOf("Operand") && !OpInfo.MIOperandInfo->arg_empty()) {
      ...
      // The argument may be a DAG or a subclass of Operand.
      if (auto *ArgDag = dyn_cast<DagInit>(Result->getArg(ArgIdx))) {
        ...
        if (auto *Operator = dyn_cast<DefInit>(ArgDag->getOperator());
          ...
          PrintFatalError(R->getLoc(), "argument #" + Twine(ArgIdx) +
                                           " operator must be " +
                                           Op->getName());
        ...
        for (unsigned SubOpIdx = 0; SubOpIdx != NumSubOps; ++SubOpIdx) {
          ...
          if (Error Err = ResOpOrErr.takeError())
            PrintFatalError(R->getLoc(), "in argument #" + Twine(ArgIdx) + "." +
                                             Twine(SubOpIdx) + ": " +
                                             toString(std::move(Err)));
          ResultOperands.push_back(*ResOpOrErr);
          ResultInstOperandIndex.push_back(std::pair(OpIdx, SubOpIdx));
        }
      } else {
